  ...
  if (stackQueryValue()) {
	...
	if (joint_dofs > 0) {
	  if (vector_table[1].length() != 6) {
		...
		errormsg << "LuaModel Error: invalid joint motion subspace description at " << this->keyStackToString() << endl;
...
bool LuaModelReadConstraintsFromTable (
  ...
  for(size_t i = 0; i < constraint_set_names.size(); ++i) {
	...
	if(!model_table["constraint_sets"][constraint_set_names[i].c_str()].exists()) {
	  std::ostringstream errormsg;
	  errormsg << "Constraint set not existing: " << constraint_set_names[i] << "." << endl;
	  throw Errors::RBDLFileParseError(errormsg.str());
	}
	...
	for(size_t ci = 0; ci < num_constraints; ++ci) {
	  ...
	  if(!model_table["constraint_sets"][constraint_set_names[i].c_str()][ci + 1]["constraint_type"].exists()) {
	  ...
	  if(constraintType == "contact") {
		if(!model_table["constraint_sets"][constraint_set_names[i].c_str()][ci + 1]["body"].exists()) {
		...
		if(!model_table["constraint_sets"][constraint_set_names[i].c_str()][ci + 1]["predecessor_body"].exists()) {
		  throw Errors::RBDLFileParseError("predecessor_body not specified.");
		}
		if(!model_table["constraint_sets"][constraint_set_names[i].c_str()][ci + 1]["successor_body"].exists()) {
		...
		if (enable_stabilization) {
			stabilization_parameter = model_table["constraint_sets"][constraint_set_names[i].c_str()][ci + 1]["stabilization_parameter"].getDefault<double>(0.1);
			if (stabilization_parameter <= 0.0) {
				std::ostringstream errormsg;
				errormsg << "Invalid stabilization parameter: " << stabilization_parameter << " must be > 0.0" << std::endl;
				throw Errors::RBDLFileParseError(errormsg.str());
			}
			double stabilization_coefficient = 1.0 / stabilization_parameter;
			constraint_sets[i].baumgarteParameters[i] = Vector2d( stabilization_coefficient, stabilization_coefficient);
		}
