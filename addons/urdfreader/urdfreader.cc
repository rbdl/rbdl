...
// ==========================================================================================
    ...
    void construct_model(Model *rbdl_model, ModelPtr urdf_model, bool floating_base,
                         bool verbose) {
      ...
      if (root->inertial.has_value()) {
        ...
        if (root_inertial_mass == 0. && root_inertial_inertia != Matrix3d::Zero()) {
          ...
          error_msg << "Error creating rbdl model! Urdf root link (" << root->name
