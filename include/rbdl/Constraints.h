...
struct RBDL_DLLAPI ConstraintSet {
  ...
  */
  ...
  /**
      ...
      @param constraintImpulsesUpd: a vector of spatial impulses generated by this
              constraint. The \f$i^{th}\f$ spatial impulse is resolved into the
              \f$i^{th}\f$ frame listed in constraintBodyFramesUpd.

      @param resolveAllInRootFrame: 
              - false: spatial impulses are resolved into the local frames attached to the bodies involved in this constraint
              - true: spatial impulses are resolved into the base frame. Note that this means that all entires in constraintBodyIdsUpd will be 0, the frames in constraintFramesUpd will have their origins at the contact frames but with directions that match the base frame.
