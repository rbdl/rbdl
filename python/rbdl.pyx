...
cdef UpdateKinematics (Model model, q, qdot, qddot):
#    q_in = VectorNd.fromPythonArray (q)
#    qdot_in = VectorNd.fromPythonArray (qdot)
#    qddot_in = VectorNd.fromPythonArray (qddot)
  
    q_in = new crbdl.VectorNd (len(q))
    qdot_in = new crbdl.VectorNd (len(qdot))
    qddot_in = new crbdl.VectorNd (len(qddot))

    for i in range (len(q)):
        q_in.data()[i] = q[i]

    for i in range (len(qdot)):
        qdot_in.data()[i] = qdot[i]
        qddot_in.data()[i] = qddot[i]
...
cdef CompositeRigidBodyAlgorithm (Model model, np.ndarray[np.float64_t, ndim=1] q, np.ndarray[np.float64_t, ndim=2] H, update_kinematics=True):
    cdef VectorNd q_in
    q_in = VectorFromPtr (model.qdot_size, q.data_ptr)
 #   cdef MatrixNd H_result = MatrixFromPtr (model.qdot_size, model.qdot_size, H.data_ptr)

    # for i in range (len(q)):
    #     q_in.data()[i] = q[i]

    # for i in range (model.qdot_size):
    #     for j in range (model.qdot_size):
    #         (&(H_in.coeff(i,j)))[0] = 0.

    # crbdl.CompositeRigidBodyAlgorithm (model.thisptr[0], q_in[0], H_in[0], update_kinematics)
