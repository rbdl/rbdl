...
struct FixedBase6DoF9DoF {
  FixedBase6DoF9DoF () {
    ...
    base_id = model->AddBody (0, Xtrans (Vector3d (0., 0., 0.)),
                              joint_rotzyx, base);
...
// 
TEST ( TestForwardDynamicsConstraintsDirectSimple_BodyToGroundConstraint ) {
  Model model;
  model.gravity = Vector3d  (0., -9.81, 0.);
  ...
  VectorNd QDDotCC = VectorNd::Constant  ((size_t) model.dof_count, 0.);
  ...
  constraint_set.AddBodyToGroundPositionConstraint(contact_body_id,
                             contact_point, normals , "ground_xyz");

  constraint_set.Bind (model);

  ConstraintSet contact_constraint_set;

  contact_constraint_set.AddContactConstraint(contact_body_id, contact_point,
  ...
  contact_constraint_set.Bind (model);
...
//  cout << constraint_set.acceleration.transpose() << endl;
  ...
  ForwardDynamicsConstraintsDirect (model, Q, QDot, Tau, contact_constraint_set,
                                    QDDotCC);
...
TEST ( TestForwardDynamicsConstraintsDirectMoving_BodyToGroundConstraint ) {
  Model model;
  model.gravity = Vector3d  (0., -9.81, 0.);
  Body base_body (1., Vector3d (0., 0., 0.), Vector3d (1., 1., 1.));
  unsigned int base_body_id = model.AddBody (0, SpatialTransform(), 
  ...
  constraint_set.AddBodyToGroundPositionConstraint(
        contact_body_id, contact_point, Vector3d (1., 0., 0.), "ground_x");
  constraint_set.AddBodyToGroundPositionConstraint (
        contact_body_id, contact_point, Vector3d (0., 1., 0.), "ground_y");
  constraint_set.AddBodyToGroundPositionConstraint (
        contact_body_id, contact_point, Vector3d (0., 0., 1.), "ground_z");
...
TEST_FIXTURE (FixedBase6DoF,
              ForwardDynamicsContactsSingleContact_BodyToGroundConstraint) {
  contact_normal.set (0., 1., 0.);
  constraint_set.AddBodyToGroundPositionConstraint(
        contact_body_id, contact_point, contact_normal);

  //ConstraintSet constraint_set_test;
  //constraint_set_test.AddBodyToGroundPositionConstraint(
  //      contact_body_id,contact_point,contact_normal);
  ...
  constraint_set.Bind (*model);  
  //constraint_set_test.Bind(*model);

  Vector3d point_accel_lagrangian, point_accel_contacts,
           point_accel_contacts_test;
  ...
  //VectorNd QDDot_contacts_test = VectorNd::Constant (model->mBodies.size() - 1, 0.);
  ClearLogOutput();
  ForwardDynamicsConstraintsDirect(*model, Q, QDot, Tau,
                                   constraint_set_lagrangian, QDDot_lagrangian);
  ClearLogOutput();
  ForwardDynamicsContactsKokkevis (*model, Q, QDot, Tau,
                                   constraint_set, QDDot_contacts);
  //ForwardDynamicsContactsKokkevis (*model, Q, QDot, Tau, constraint_set_test, QDDot_contacts_test);
//  cout << LogOutput.str() << endl;
  ClearLogOutput();

  point_accel_lagrangian = CalcPointAcceleration (*model, Q, QDot,
                        QDDot_lagrangian, contact_body_id, contact_point, true);
  point_accel_contacts = CalcPointAcceleration (*model, Q, QDot, QDDot_contacts,
                                          contact_body_id, contact_point, true);

  //point_accel_contacts_test = CalcPointAcceleration (*model, Q, QDot, QDDot_contacts_test, contact_body_id, contact_point, true);

  //CHECK_ARRAY_CLOSE(point_accel_contacts.data(),point_accel_contacts_test.data(),3,TEST_PREC);

  CHECK_CLOSE ( constraint_set_lagrangian.force[0],
                constraint_set.force[0], TEST_PREC);
  ...
  CHECK_ARRAY_CLOSE (point_accel_lagrangian.data(),
                     point_accel_contacts.data(), 3, TEST_PREC);

  CHECK_ARRAY_CLOSE (QDDot_lagrangian.data(),
                     QDDot_contacts.data(),
                     QDDot_lagrangian.size(), TEST_PREC);
}


TEST_FIXTURE (FixedBase6DoF, ForwardDynamicsContactsSingleContactRotated_BodyToGroundConstraint) {
  Q[0] = 0.6;
  Q[3] =   M_PI * 0.6;
  Q[4] = 0.1;
  ...
  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, contact_point, contact_normal);
  ConstraintSet constraint_set_lagrangian = constraint_set.Copy();
...
TEST_FIXTURE (FixedBase6DoF, ForwardDynamicsContactsSingleContactRotatedMoving_BodyToGroundConstraint) {
  Q[0] = 0.6;
  Q[3] =   M_PI * 0.6;
  Q[4] = 0.1;
...
TEST_FIXTURE (FixedBase6DoF, ForwardDynamicsContactsOptDoubleContact_BodyToGroundConstraint) {
  ConstraintSet constraint_set_lagrangian;

  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, Vector3d (1., 0., 0.), contact_normal);
  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, Vector3d (0., 1., 0.), contact_normal);
...
TEST_FIXTURE (FixedBase6DoF, ForwardDynamicsContactsOptDoubleContactRepeated_BodyToGroundConstraint) {
...
TEST_FIXTURE (FixedBase6DoF, ForwardDynamicsContactsOptMultipleContact_BodyToGroundConstraint) {
  ConstraintSet constraint_set_lagrangian;

  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, contact_point, Vector3d (1., 0., 0.));
  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, contact_point, Vector3d (0., 1., 0.));
...
TEST_FIXTURE (FixedBase6DoF9DoF, ForwardDynamicsContactsOptMultipleContactsMultipleBodiesMoving_BodyToGroundConstraint) {
  ConstraintSet constraint_set_lagrangian;

  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, contact_point, Vector3d (1., 0., 0.));
  constraint_set.AddBodyToGroundPositionConstraint (contact_body_id, contact_point, Vector3d (0., 1., 0.));
  constraint_set.AddBodyToGroundPositionConstraint (child_2_id, contact_point, Vector3d (0., 1., 0.));
...
TEST_FIXTURE (FixedBase6DoF9DoF, ForwardDynamicsContactsOptMultipleContactsMultipleBodiesMovingAlternate_BodyToGroundConstraint) {
  ConstraintSet constraint_set_lagrangian;
...
TEST_FIXTURE (FixedBase6DoF12DoFFloatingBase, ForwardDynamicsContactsMultipleContactsFloatingBase_BodyToGroundConstraint) {
  ConstraintSet constraint_set_lagrangian;
...
TEST_FIXTURE (Human36, ForwardDynamicsContactsFixedBody_BodyToGroundConstraint) {
  ...
  constraint_upper_trunk.AddBodyToGroundPositionConstraint (body_id_3dof[BodyUpperTrunk], Vector3d (1.1, 2.2, 3.3), Vector3d (1., 0., 0.));
...
TEST_FIXTURE (Human36, ForwardDynamicsContactsImpulses_BodyToGroundConstraint) {
  VectorNd qddot_lagrangian (VectorNd::Zero(qddot.size()));
  ...
  constraint_upper_trunk.AddBodyToGroundPositionConstraint (body_id_3dof[BodyFootLeft], heel_point, normals);
  constraint_upper_trunk.AddBodyToGroundPositionConstraint (body_id_3dof[BodyFootRight], heel_point, normals);
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d (1., 0., 0.));
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d (0., 1., 0.));
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootLeft], heel_point, Vector3d (0., 0., 1.));
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d (1., 0., 0.));
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d (0., 1., 0.));
  //constraint_upper_trunk.AddBodyToGroundPositionConstraint(body_id_3dof[BodyFootRight], heel_point, Vector3d (0., 0., 1.));
